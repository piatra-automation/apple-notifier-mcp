 ...
 */
async function checkTerminalNotifierAvailability(): Promise<boolean> {
  try {
  ...
  try {
    ...
    if (!isAvailable) {
      throw new NotificationError(
        ...
        'terminal-notifier is not installed. Please install it using: brew install terminal-notifier'
      );
    }
    ...
    if (err.message.includes('terminal-notifier: command not found')) {
      ...
      throw new NotificationError(
        ...
        'Permission denied when trying to send notification. Please check macOS notification settings.'
      );
    } else {
      throw new NotificationError(
